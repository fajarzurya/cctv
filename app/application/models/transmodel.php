<?php 
  ...
  class Transmodel extends CI_Model
  {
	 ...
	 {
	   return $this->db->query('SELECT * FROM PEMELIHARAAN');
	 }
	 ...
	 {
	   return $this->db->query('SELECT * FROM PELEPASAN');
	 }
	 public function pelepasan_id($id)//select
	 {
	   return $this->db->query("SELECT * FROM PELEPASAN WHERE ID_PELEPASAN = '".$id."'");
	 }
	 public function tambah_lepas($data)//insert
	 {
   	   $this->db->insert('PELEPASAN',$data); 
	 }
	 public function edit_lepas($id,$data)//edit
	 {
	   $this->db->where('ID',$id);
   	   $this->db->update('PELEPASAN',$data); 
	 }
	 public function delete_lepas($id)//delete
	 {
	   $this->db->query("DELETE FROM PELEPASAN
						   WHERE ID_PELEPASAN=
						  ".$id); 
	 }
  }
